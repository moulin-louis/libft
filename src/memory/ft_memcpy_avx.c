...
static inline void align_memory_sse_asm(char** d, const char** s, uint64_t* n) {
	asm volatile
	(
		"movdqu  (%1), %%xmm0\n\t"
		"movdqu  %%xmm0, (%0)\n\t"
...
static  void align_memory_sse(char** d, const char** s, uint64_t* n) {
	while ((((uintptr_t)*d & 0x1F) || ((uintptr_t)*s & 0x1F)) && *n >= 16)
		align_memory_sse_asm(d, s, n);
}

static void copy_chunks_avx_asm(char* d, const char* s, uint64_t* num_chunks) {
	asm volatile (
		"1:                             \n\t"
		"vmovdqa  (%1), %%ymm0          \n\t"
		"vmovntdq  %%ymm0, (%0)         \n\t"
...
static inline void copy_chunks_avx(char* d, const char* s, uint64_t* n) {
	uint64_t num_chunks;
